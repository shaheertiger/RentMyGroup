...
export const Features: React.FC = () => {
  ...
  return (
    <section id="features" className="py-24 md:py-40 bg-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#4f46e5 1px, transparent 0)', backgroundSize: '40px 40px' }} aria-hidden="true"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between mb-24 gap-12 text-center lg:text-left">
           <div className="max-w-3xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-[10px] font-black uppercase tracking-[0.2em] mb-8">
                <BarChart3 size={12} aria-hidden="true" /> Beat the Facebook Algorithm
              </div>
              <h2 className="text-5xl md:text-7xl lg:text-8xl font-display font-black text-slate-900 mb-8 tracking-tighter leading-[1.05]">
                ...
                <span className={`${viewMode === 'control' ? 'text-indigo-600' : 'text-red-500'} transition-colors duration-500 relative inline-block py-1`}>
                  Algorithm.
                  <svg className="absolute -bottom-2 left-0 w-full" viewBox="0 0 358 12" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              ...
              <p className="text-slate-500 text-xl md:text-2xl font-medium leading-relaxed max-w-2xl mx-auto lg:mx-0">
                Social feeds are a lottery. Renting community attention is a verified business strategy. We put your brand in the <strong>Primary Slot</strong> of high-intent local hubs.
              </p>
           </div>
           
           {/* Mode Switcher */}
           <div className="flex flex-col gap-4 p-2 bg-slate-900 rounded-[2.5rem] border border-slate-800 w-fit shrink-0 shadow-2xl relative overflow-hidden mx-auto lg:mx-0">
              <div className="flex p-1 bg-white/5 rounded-2xl relative z-10" role="group" aria-label="Switch advertising mode">
                <button 
                ...
                >
                  <ShieldAlert size={14} aria-hidden="true" /> Algorithmic Chaos
                </button>
                <button 
                ...
                >
                  <Target size={14} aria-hidden="true" /> RMG Control
                </button>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-fr">
           ...
           <article className={`md:col-span-12 lg:col-span-8 bg-slate-950 rounded-[3rem] p-10 md:p-16 text-white group relative overflow-hidden border border-white/5 flex flex-col justify-between transition-all duration-700 min-h-[500px] ${viewMode === 'chaos' ? 'ring-2 ring-red-500/20' : 'ring-2 ring-indigo-500/20'}`}>
              <div className="relative z-20 max-w-lg">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-10 shadow-2xl transition-colors duration-500 ${viewMode === 'control' ? 'bg-indigo-600' : 'bg-red-500'}`}>
                  {viewMode === 'control' ? <MessageCircle size={28} aria-hidden="true" /> : <ShieldAlert size={28} aria-hidden="true" />}
                </div>
                ...
                <p className="text-slate-400 text-lg md:text-xl font-medium leading-relaxed">
                  {viewMode === 'control' 
                    ? "Pinned messages aren't 'ads'—they're part of the group's architecture. They aren't scrolled past; they're referenced daily by thousands of active neighbors in local groups." 
              ...
              <div className="mt-12 relative z-20">
                 <button className="flex items-center gap-2 text-indigo-400 font-bold hover:text-indigo-300 transition-colors">
                    Learn about Fixed-Rent Attention <ArrowRight size={18} aria-hidden="true" />
                 </button>
              </div>
           </article>

           {/* Card 2: Activation Time */}
           <div className="md:col-span-12 lg:col-span-4 bg-indigo-600 rounded-[3rem] p-1 border border-white/10 shadow-2xl shadow-indigo-600/20 flex flex-col justify-center items-center relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-500 to-indigo-700" aria-hidden="true"></div>
              ...
              <div className="bg-white/10 backdrop-blur-2xl rounded-[3rem] p-10 border border-white/20 shadow-2xl relative z-10 w-[85%] flex flex-col items-center">
                 <div className="text-center w-full">
                    <div className="text-[11px] font-black text-white/60 uppercase tracking-[0.3em] mb-8">Average Ad Approval</div>
                    ...
                    <div className="w-full h-1.5 bg-white/20 rounded-full relative overflow-hidden" aria-label="Progress bar">
                       <div className="absolute top-0 left-0 h-full bg-white w-[78%] rounded-full shadow-[0_0_15px_rgba(255,255,255,0.5)]"></div>
                    </div>
                 </div>
              </div>

              <div className="absolute -top-20 -right-20 w-64 h-64 bg-white/10 rounded-full blur-[100px] animate-pulse" aria-hidden="true"></div>
              <div className="absolute -bottom-20 -left-20 w-64 h-64 bg-indigo-400/20 rounded-full blur-[100px]" aria-hidden="true"></div>
           </div>

           {/* Card 3: Hyper-Local Targeting */}
           <article className="md:col-span-6 lg:col-span-5 bg-slate-50 rounded-[3rem] p-10 border border-slate-200 group flex flex-col justify-between overflow-hidden relative">
              <div className="absolute -top-10 -right-10 opacity-[0.05] group-hover:rotate-12 transition-transform duration-700" aria-hidden="true">
                <MapPin size={200} />
              </div>
              <div>
                ...
                <h3 className="text-3xl font-black text-slate-900 mb-4 font-display tracking-tight leading-none">Hyper-Local Targeting</h3>
                <p className="text-slate-500 text-lg font-medium leading-relaxed">
                  Target exactly where your customers live. No wasted spend on users 20 miles away. Secure <strong>Facebook group sponsorships</strong> in your specific zip codes.
                </p>
              </div>
           ...
           <article className="md:col-span-6 lg:col-span-7 bg-white rounded-[3rem] p-10 border border-slate-200 group flex flex-col md:flex-row items-center gap-10">
              <div className="flex-1">
                 ...
                 <h3 className="text-3xl font-black text-slate-900 mb-4 font-display tracking-tight leading-none">Verified Neighbors</h3>
                 <p className="text-slate-500 text-lg font-medium leading-relaxed">
                   We vet every group for bot activity. Your ads are seen by real neighbors, not fake accounts. Community management at its most transparent.
                 </p>
              </div>
              <div className="shrink-0 flex -space-x-4">
                 {avatars.map((url, i) => (
                   <img key={i} src={url} className="w-14 h-14 rounded-full border-4 border-white shadow-xl object-cover" alt="Verified neighbor avatar" />
                 ))}
              </div>
           </article>
